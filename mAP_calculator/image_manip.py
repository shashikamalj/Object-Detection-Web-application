...
class ImageManipulator:
    ...
    def calc_iou(self, gt_bbox, pred_bbox):
        ...
        if (x_topleft_gt > x_bottomright_gt) or (y_topleft_gt > y_bottomright_gt):
            # print(x_topleft_gt,x_bottomright_gt)
            # print(y_topleft_gt,y_bottomright_gt)
        ...
        if y_bottomright_gt < y_topleft_p:
            # If bottom right of y-coordinate
            #  GT  bbox is less than or above the top left of y coordinate of  the predicted BBox

            return 0.0
        if x_topleft_gt > x_bottomright_p:
            # If bottom right of x-coordinate  GT  bbox is
            # greater than or below the bottom right  of x coordinate of  the predcited BBox
            return 0.0
        if y_topleft_gt > y_bottomright_p:
            # If bottom right of y-coordinate  GT  bbox
            # is greater than or below the bottom right  of y coordinate of  the predcited BBox
    ...
    def get_avg_precision_at_iou(self, gt_boxes, pred_bb, iou_thr=0.5):
        ...
        sorted_model_scores = sorted(
            model_scores.keys())  # Sort the predicted boxes in descending order (lowest scoring boxes first):
        ...
        for ithr, model_score_thr in enumerate(sorted_model_scores[:-1]):
            ...
            for img_id in img_ids:
                ...
                pred_boxes_pruned[img_id]['boxes'] = pred_boxes_pruned[img_id]['boxes'][
                                                     start_idx:]  # Recalculate image results for this image
                # print(img_id)
                img_results[img_id] = self.get_single_image_results(gt_boxes_img, pred_boxes_pruned[img_id]['boxes'],
                                                                    iou_thr=0.5)  # calculate precision and recall
            prec, rec = self.calc_precision_recall(img_results)
            # np.concatenate(precisions,prec)
            # print("Prec......",prec)
